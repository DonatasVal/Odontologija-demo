...
const PUBLIC_LEADS_STORAGE_KEY = "vardenis_public_registration_leads";
...
function currentMonthKey() {
  return dateFromToday(0).slice(0, 7);
}
...
const initialRegistrations = [
  { id: "r1", clientId: "c1", date: dateFromToday(0), time: "08:00", service: "Individuali treniruotė", status: "Aktyvus", goal: "Viršutinė kūno dalis ir core" },
  { id: "r2", clientId: "c2", date: dateFromToday(0), time: "09:30", service: "Treniruočių abonementas", status: "Patvirtinta", goal: "Pradinis pokalbis ir grafiko suderinimas" },
  { id: "r3", clientId: "c6", date: dateFromToday(0), time: "11:00", service: "Individuali treniruotė", status: "Priimtas", goal: "Pirma treniruotė, judesių įvertinimas" },
  { id: "r4", clientId: "c3", date: dateFromToday(0), time: "16:30", service: "Nuotolinė priežiūra", status: "Naujas", goal: "Aptarti tikslą ir sporto salės įrangą" },
  { id: "r5", clientId: "c7", date: dateFromToday(0), time: "18:00", service: "Treniruočių abonementas", status: "Aktyvus", goal: "Laikysena, core, mobilumas" },
  { id: "r6", clientId: "c4", date: dateFromToday(0), time: "19:30", service: "Treniruočių planas", status: "Užklausa", goal: "Aptarti plano struktūrą" },

  { id: "r7", clientId: "c8", date: dateFromToday(1), time: "08:00", service: "Treniruočių planas", status: "Naujas", goal: "Surinkti informaciją planui" },
  { id: "r8", clientId: "c1", date: dateFromToday(1), time: "18:00", service: "Individuali treniruotė", status: "Aktyvus", goal: "Kojos ir technika" },
  { id: "r9", clientId: "c2", date: dateFromToday(2), time: "16:30", service: "Treniruočių abonementas", status: "Patvirtinta", goal: "Treniruotė pagal mėnesinį grafiką" },
  { id: "r10", clientId: "c5", date: dateFromToday(2), time: "19:30", service: "Mitybos konsultacija", status: "Atmestas", goal: "Konsultacijos užklausa atmesta" },
  { id: "r11", clientId: "c7", date: dateFromToday(3), time: "08:00", service: "Treniruočių abonementas", status: "Aktyvus", goal: "Mobilumas ir full body" },
  { id: "r12", clientId: "c6", date: dateFromToday(4), time: "11:00", service: "Individuali treniruotė", status: "Priimtas", goal: "Bazinių judesių technika" },
  { id: "r13", clientId: "c1", date: dateFromToday(7), time: "08:00", service: "Individuali treniruotė", status: "Aktyvus", goal: "Viršutinė kūno dalis" },
  { id: "r14", clientId: "c2", date: dateFromToday(8), time: "09:30", service: "Treniruočių abonementas", status: "Patvirtinta", goal: "Jėgos treniruotė" },
  { id: "r15", clientId: "c7", date: dateFromToday(10), time: "18:00", service: "Treniruočių abonementas", status: "Aktyvus", goal: "Core ir laikysena" },
  { id: "r16", clientId: "c4", date: dateFromToday(14), time: "16:30", service: "Treniruočių planas", status: "Užklausa", goal: "Plano korekcija" },
];

const initialBlocked = [
  { id: "b1", date: dateFromToday(0), time: "06:30", reason: "Treneris užimtas / pasiruošimas" },
  { id: "b2", date: dateFromToday(0), time: "12:30", reason: "Pertrauka / administracinis laikas" },
  { id: "b3", date: dateFromToday(5), time: "12:30", reason: "Asmeninis užimtumas" },
  { id: "b4", date: dateFromToday(12), time: "16:30", reason: "Mokymai" },
];
...
function leadToClient(lead) {
  const clientId = `lead-client-${lead.id}`;

  return {
    id: clientId,
    name: lead.client?.name || "Naujas klientas",
    phone: lead.client?.phone || "",
    email: lead.client?.email || "",
    status: "Naujas",
    packageName: lead.service?.title || "Registracija",
    goal: lead.client?.goal || "",
    frequency: lead.schedule?.requiresSchedule
      ? `${lead.schedule?.date || "Derinama"} ${lead.schedule?.time || ""}`.trim()
      : `${lead.client?.preferredDays || "Derinama"} · ${lead.client?.preferredTime || "Derinama"}`,
    health: lead.client?.healthIssues || "Nenurodyta.",
    notes: lead.client?.notes || "Registracija gauta iš viešos formos.",
    plan: "Laukiama trenerio peržiūros.",
  };
}

function leadToRegistration(lead) {
  const clientId = `lead-client-${lead.id}`;

  return {
    id: `lead-registration-${lead.id}`,
    clientId,
    date: lead.schedule?.date || dateFromToday(0),
    time: lead.schedule?.time || "Derinama",
    service: lead.service?.title || "Registracija",
    status: "Naujas",
    goal: lead.client?.goal || "",
  };
}

function mergePublicLeads(state) {
  const leads = safeJsonParse(localStorage.getItem(PUBLIC_LEADS_STORAGE_KEY), []);
  if (!Array.isArray(leads) || leads.length === 0) return state;

  const existingClientIds = new Set(state.clients.map((client) => client.id));
  const existingRegistrationIds = new Set(state.registrations.map((registration) => registration.id));

  const newClients = [];
  const newRegistrations = [];

  leads.forEach((lead) => {
    const client = leadToClient(lead);
    const registration = leadToRegistration(lead);

    if (!existingClientIds.has(client.id)) newClients.push(client);
    if (!existingRegistrationIds.has(registration.id)) newRegistrations.push(registration);
  });
...
function loadInitialState() {
  try {
    ...
    return mergePublicLeads(normalizeState(JSON.parse(saved)));
  } catch {
    return mergePublicLeads(normalizeState({
...
export default function TrainerAdmin() {
  ...
  const [selectedDate, setSelectedDate] = useState(() => dateFromToday(0));
