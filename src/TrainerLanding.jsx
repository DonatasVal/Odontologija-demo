...
const ADMIN_STORAGE_KEY = "vardenis_admin_stage_3_cards_pdf_preview";
const PUBLIC_LEADS_STORAGE_KEY = "vardenis_public_registration_leads";

// Realiam siuntimui įrašomas webhook endpoint per Vercel env:
// VITE_REGISTRATION_WEBHOOK_URL=https://jusu-formos-endpointas
const REGISTRATION_WEBHOOK_URL = (import.meta.env?.VITE_REGISTRATION_WEBHOOK_URL || "").trim();
...
function savePublicLead(payload) {
  const current = safeJsonParse(localStorage.getItem(PUBLIC_LEADS_STORAGE_KEY), []);
  const next = Array.isArray(current) ? current : [];
  localStorage.setItem(PUBLIC_LEADS_STORAGE_KEY, JSON.stringify([payload, ...next]));
}

function saveLeadToExistingAdminState(payload) {
  const saved = safeJsonParse(localStorage.getItem(ADMIN_STORAGE_KEY), null);
  if (!saved || !Array.isArray(saved.clients) || !Array.isArray(saved.registrations)) return;

  const clientId = `lead-client-${payload.id}`;
  const registrationId = `lead-registration-${payload.id}`;
  ...
  const client = {
    ...
    status: "Naujas",
    packageName: payload.service.title,
    goal: payload.client.goal,
    frequency: payload.schedule.requiresSchedule
      ? `${payload.schedule.date} ${payload.schedule.time}`
      : `${payload.client.preferredDays || "Derinama"} · ${payload.client.preferredTime || "Derinama"}`,
    health: payload.client.healthIssues,
    notes: payload.client.notes || "Registracija gauta iš viešos formos.",
  ...
  const registration = {
    id: registrationId,
    clientId,
    date: payload.schedule.date || dateFromToday(0),
    time: payload.schedule.time || "Derinama",
  ...
  localStorage.setItem(
    ...
    JSON.stringify({
      ...saved,
      clients: [client, ...saved.clients],
      registrations: [registration, ...saved.registrations],
    })
  );
}

async function sendRegistrationWebhook(payload) {
  ...
  if (!response.ok) {
    throw new Error("Webhook grąžino klaidą");
  }

  return { sent: true, mode: "webhook" };
}

function buildRegistrationPayload({ form, selectedService, selectedDate, selectedTime, requiresSchedule }) {
  ...
  return {
    ...
    schedule: {
      requiresSchedule,
      date: requiresSchedule ? selectedDate : "",
      time: requiresSchedule ? selectedTime : "",
    ...
    client: {
      ...
      preferredDays: form.preferredDays.trim(),
...
function BookingModal({ onClose }) {
  ...
  const [selectedDate, setSelectedDate] = useState(() => dateFromToday(1));
  ...
  const minBookingDate = useMemo(() => dateFromToday(0), []);
  ...
  const canContinue = useMemo(() => {
    ...
    return Boolean(form.name.trim() && isValidPhone(form.phone) && form.consent);
  ...
  async function handleSubmit(event) {
    ...
    if (!isValidPhone(form.phone)) {
      setSubmitError("Įveskite teisingą telefono numerį, pvz. +370 600 00000.");
      return;
    }
    ...
    try {
      ...
      // Išsaugoma demo/admin peržiūrai tame pačiame naršyklės įrenginyje.
      savePublicLead(payload);
      saveLeadToExistingAdminState(payload);

      const result = await sendRegistrationWebhook(payload);
      setSubmissionMode(result.mode);
      setIsSubmitted(true);
    } catch (error) {
      setSubmitError("Registracijos nepavyko išsiųsti. Patikrinkite ryšį arba susisiekite su treneriu tiesiogiai.");
    } finally {
      ...
      <div className="modal">
        ...
        {!isSubmitted ? (
              ...
              <section className="wizard-panel">
                  ...
                  <>
                    ...
                    {requiresSchedule ? (
                      <>
                        ...
                        <input className="input-field" type="date" min={minBookingDate} value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} style={{ marginBottom: 12 }} />
                ...
                {step === 4 && (
                  <>
                    ...
                    <div className="form-grid">
                      ...
                      <div>
                        <input
                          className={`input-field ${form.phone && !isValidPhone(form.phone) ? "danger-border" : ""}`}
                          type="tel"
                          inputMode="tel"
                          autoComplete="tel"
                        ...
                        {form.phone && !isValidPhone(form.phone) && (
                          <div className="field-error">Įveskite telefono numerį, pvz. +370 600 00000.</div>
                        )}
                      </div>
                ...
                <div className="wizard-actions">
                  ...
                  {step < wizardSteps.length - 1 ? (
                    ...
                    <button className="btn btn-dark" type="submit" disabled={!canContinue || isSubmitting}>
                      {isSubmitting ? "Siunčiama..." : "Pateikti registraciją"}
                    </button>
                  )}
                </div>
              </section>
          ...
          <div className="success-box">
            <h2>{submissionMode === "webhook" ? "Užklausa sėkmingai išsiųsta" : "Užklausa išsaugota"}</h2>
            <p>{requiresSchedule ? `Pasirinktas laikas: ${selectedDate} · ${selectedTime}` : "Laikai ir treniravimo planas bus suderinti po pokalbio."}</p>
            {submissionMode !== "webhook" && (
              <p className="field-hint">Demo režimu užklausa išsaugoma naršyklėje. Realiam siuntimui įjunkite VITE_REGISTRATION_WEBHOOK_URL.</p>
            )}
  ...
  return (
    <main className="landing">
      ...
      <footer className="container footer">
        © {new Date().getFullYear()} Vardenis Pavardenis. Demonstracinė asmeninio trenerio svetainė.
      </footer>
